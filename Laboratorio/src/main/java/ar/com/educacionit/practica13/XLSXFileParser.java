...
public class XLSXFileParser extends BaseFile implements papaparse<Collection<Articulos>> {
	...
	public Collection<Articulos> parse() throws Exception {
		...
		try(
			   ...
			   int hjs = workbook.getNumberOfSheets();
			   
		  for(int i=0 ;i<hjs;i++) {
			Sheet hojas = workbook.getSheetAt(i);
			
			//collection
			Iterator<Row> filasDeLaHojax = hojas.iterator();
			
			boolean primerFila = true;
			//TITULO	CODIGO	PRECIO	STOCK	MARCASID	CATEGORIASID
			
			while(filasDeLaHojax.hasNext()) {
				Row filaActual = filasDeLaHojax.next();
				if(primerFila) {
					primerFila = false;
					continue;
				}
			...
			throw new Exception("No se ha podido parsear el archivo: " + getFilePath(),fnfe);
		}
